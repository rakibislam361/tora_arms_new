(function ($) {
    ...
    $("#preloader").delay(500).fadeOut(300).addClass('loaded');
...
$(function () {
    ...
    function hoverable_menu_helpers() {
        const logoWidth = $(".navbar-brand").width() - 100;
        const container = $(".container").width() - 200;
        $(".mega-dropdown-area").css({
            left: "-" + logoWidth + "px",
            width: container + "px"
        });
    }
    ...
    if ($(window).width() < 576) {
        ...
        hoverable_menu_helpers();
        $(".hovarable-tab>a").on("click", function () {
            window.location.href = $(this).attr("data-link");
        });
    }

    $("#mobile-toggler").on("click", function () {
        $(".mega-dropdown-area").addClass("active show");
    });


    $(".hovarable-tab a").hover(function () {
        var tabContent = $('#v-pills-tabContent');
        tabContent.find('.tab-pane').removeClass('show active');
        var controls = $(this).attr('aria-controls');
        $('#' + controls).addClass('show active');
        $(this).closest('#v-pills-tab').find('.nav-link').removeClass('active');
        $(this).addClass('active');
    });


    // console.log($('.hovarable-tab').innerHeight());

    $('.mega-dropdown .tab-content').css({height: $('.hovarable-tab').innerHeight()});



    $("ul.navbar-nav li.hovarable").hover(
        function () {
            $(this)
                .find(".dropdown-menu")
                .stop(true, true)
